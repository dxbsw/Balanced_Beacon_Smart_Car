...
/*LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL
@函数名称：void Spi_Hard_Init(unsigned short type)
@功能说明：硬件SPI初始化
@参数说明：无
@函数返回：无
@修改时间：2022/04/05
...
void Spi_Hard_Init(void)
{
	...
	SPI_InitStruct.SPI_BaudRatePrescaler = (SPI_BaudRatePrescaler_TypeDef)0x8;
...
void Spi_Gpio_Config(void)
{
  ...
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOE, ENABLE);   //GPIOE clk enable
	
	GPIO_PinAFConfig(GPIOE, GPIO_PinSource2, GPIO_AF_5);		//SCK
...
void TFTSPI_Init(unsigned short type)
{
	...
	Spi_Hard_Init();
	Spi_Gpio_Config();
      //  DMA_CameraInitConfig(TFT_DMA_CH,(u32)Image_Use, (u32), LCDH*LCDW);
      //  DMA_CameraTriggerTimerInit(, TFTSPI_SCK); //GPIO触发TIM，只有引脚固定的几个可以选择  
  ...
  if(type)TFTSPI_Write_Byte(0xC0);            //竖屏显示          //MX=1,MY=1   
  else TFTSPI_Write_Byte(0xA0);            	  //横屏显示
...
@备    注：无
QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ*/
void TFTSPI_Write_Word(unsigned short data)
{
#if use_heard_spi		                      //硬件SPI通信
  unsigned char datH = data&0xff;
	unsigned char datL = data>>8;

	SPI_SendData(SPI2, datL);
	SPI_SendData(SPI2, datH);
	while(!(SPI2->CSTAT & SPI_FLAG_TXEPT));
  while(!(SPI2->CSTAT & SPI_CSTAT_RXAVL));
#else                                     //软件SPI通信
  unsigned short i;
...
/*LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL
@函数名称：void TFTSPI_Draw_Part(unsigned short xs,unsigned short ys,unsigned short xe,unsigned short ye,unsigned short color_dat)
@功能说明：填充矩形区域
...
/*LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL
@函数名称：void TFTSPI_BinRoad3(unsigned short wide_start, unsigned short high_start, unsigned short high, unsigned short wide )
@功能说明：函数可以任意截取画面
@参数说明：high_start ： 显示图像开始位置wide_start：显示图像开始位置 high： 显示图像高度 wide：显示图像宽度 Pixle：显示图像数据地址
@函数返回：无
@修改时间：2022/02/24
...
/*LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL
@函数名称：void TFTSPI_BinRoad2(unsigned short wide_start, unsigned short high_start, unsigned short high, unsigned short wide )
@功能说明：函数可以任意截取画面
@参数说明：high_start ： 显示图像开始位置wide_start：显示图像开始位置 high： 显示图像高度 wide：显示图像宽度 Pixle：显示图像数据地址
@函数返回：无
@修改时间：2022/02/24
...
{	
  ...
  TFTSPI_CLS(u16RED);    //红色屏幕	
	delayms(500);
	TFTSPI_CLS(u16ORANGE); //橘色屏幕	
	delayms(500);
	TFTSPI_CLS(u16YELLOW); //黄色屏幕	
	delayms(500);
	TFTSPI_CLS(u16GREEN);  //绿色屏幕	
	delayms(500);
	TFTSPI_CLS(u16BLUE);   //蓝色屏幕	
	delayms(500);
	TFTSPI_CLS(u16CYAN);   //蓝绿色屏幕	
	delayms(500);
	TFTSPI_CLS(u16PURPLE); //紫色屏幕	
	delayms(500);
	TFTSPI_CLS(u16WHITE);  //白色屏幕	
	
	TFTSPI_Draw_Part(0, 113, 159, 132, u16RED);
	delayms(500);
	TFTSPI_Draw_Part(0, 94, 159, 112, u16ORANGE);
	delayms(500);
	TFTSPI_Draw_Part(0, 75, 159, 93, u16YELLOW);
	delayms(500);
	TFTSPI_Draw_Part(0, 56, 159, 74, u16GREEN);
	delayms(500);
	TFTSPI_Draw_Part(0, 37, 159, 55, u16BLUE);
	delayms(500);
	TFTSPI_Draw_Part(0, 18, 159, 36, u16CYAN);
	delayms(500);
  TFTSPI_Draw_Part(0, 0, 159, 17, u16PURPLE);
	delayms(500);
